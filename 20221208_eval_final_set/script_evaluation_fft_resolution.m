...
evalIdSet = [8, 84, 10];
resolutionSet = [20, 10, 5, 2, 0.1];

nRepeat = 100;
newUnit = 40;

for evalId = evalIdSet
    for resolution = resolutionSet
        disp(['=================', num2str([evalId, resolution]), '=================']);
        morePostfix = ['_', num2str(resolution), 'Hz_resolution'];
...
%         config.calib.fH = 2500;
        ...
        config.extract.nfft = config.data.rate / resolution; % Zero padding
