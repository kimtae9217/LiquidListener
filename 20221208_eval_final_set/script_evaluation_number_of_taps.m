...
evalIdSet = [8, 84, 10];
nTrialSet = [1, 2, 3, 5, 7, 9, 11, 13, 15];

nRepeat = 100;
newUnit = 40;

for evalId = evalIdSet
    for nTrial = nTrialSet
        disp(['=================', num2str([evalId, nTrial]), '=================']);
        morePostfix = ['_', num2str(nTrial), '_trials'];
        ...
        config.calib.nTrialPerTrain = min([nTrial, config.data.nTrialPerLoc * length(config.calib.locSet)]);
%         config.calib.fL = 100;
%         config.calib.fH = 2500;
        ...
        config.eval.nTrial = min([nTrial, config.data.nTrialPerLoc * length(config.eval.locSet)]);
