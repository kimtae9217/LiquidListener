...
% evalIdSet = [77:82, 8, 84, 10];
evalIdSet = [8, 84, 10];
calibPointSet = [3, 4, 5, 7, 9];

nRepeat = 100;
newUnit = 40;
for evalId = evalIdSet
    for calibPoint = calibPointSet
        disp(['=================', num2str([evalId, calibPoint]), '=================']);
        morePostfix = ['_', num2str(calibPoint), '_calib_points'];
        ...
        config.calib.nTrain = min(calibPoint, config.data.nData / config.data.nTrial);
...
%         disp(unique([test(:).amount]))
        ...
        for cnt = 1:nRepeat
            ...
            config.eval.amountSet(config.eval.amountSet == 0 | config.eval.amountSet == cup.maxAmount) = [];
        ...
        save([config.result.path, file.name, morePostfix, '_result.mat'], 'result');
    end
end
